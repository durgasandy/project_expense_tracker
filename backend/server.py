from fastapi import FastAPI, HTTPException
import datetime
import db_helper
from pydantic import BaseModel
from typing import List
from fastapi import Query


app=FastAPI()
class Expense(BaseModel):
    amount: float
    category: str
    notes:str
class DateRange(BaseModel):
    start_date: datetime.date
    end_date: datetime.date



@app.get("/expenses/{expense_date}",response_model=List[Expense])
def get_expense(expense_date:datetime.date):
    expenses=db_helper.fetch_expenses_for_date(expense_date)
    if expenses is None:
        raise HTTPException(status_code=500, detail="Failed to retrieve expenses from the database.")
    return expenses



@app.post("/expenses/{expense_date}")
def post_expense(expense_date:datetime.date,expenses:List[Expense]):
    db_helper.delete_expenses_for_date(expense_date)
    for expense in expenses:
        db_helper.insert_expense(expense_date,expense.amount,expense.category,expense.notes )
    return {"message":"expense inserted successfully "}

@app.post("/analytics/")

def get_analytics(date_range: DateRange):
    data = db_helper.fetch_expense_summary(date_range.start_date, date_range.end_date)
    if data is None:
        raise HTTPException(status_code=500, detail="Failed to retrieve expense summary from the database.")


    total = sum([row['total'] for row in data])

    breakdown = {}
    for row in data:
        percentage = (row['total']/total)*100 if total != 0 else 0
        breakdown[row['category']] = {
            "total": row['total'],
            "percentage": percentage
        }

    return breakdown

@app.get("/analytics_monthly/")
def get_analytics_monthly():
    data =db_helper.fetch_expense_monthly()
    if data is None:
        raise HTTPException(status_code=500, detail="Failed to retrieve expenses from the database.")
    monthly = []
    for row in data:
        monthly.append({
            "month": row["month"],
            "total_expense": float(row["total_expense"])
        })

    return monthly

@app.get("/analytics_weekly/{expense_date}")
def get_weekly_analytics(expense_date: str):

    data = db_helper.fetch_expense_weekly(expense_date)
    if data is None:
        raise HTTPException(status_code=500, detail="Failed to retrieve expenses from the database.")
    weekly = []
    for row in data:
        weekly.append({
            "month": row["month"],
            "week_of_month":row["week_of_month"],
            "total_expense": float(row["total_expense"])
        })
    return weekly